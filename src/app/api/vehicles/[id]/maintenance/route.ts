...
import { createMaintenance, VehicleNotFoundError } from "@/server/services/vehicle.service";

const schema = z.object({
  tipo: z.enum(["PREVENTIVA", "CORRETIVA"]),
  data: z.coerce.date().optional(),
  km: z.coerce.number().int().optional(),
  descricao: z.string().optional(),
  custoCentavos: z.coerce.number().int().optional(),
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const maintenance = await createMaintenance(params.id, companyId, parsed.data);
    return NextResponse.json({
      success: true,
      data: maintenance,
      error: null,
      message: "Manutenção registrada. Veículo marcado como em manutenção.",
    });
  } catch (err) {
    ...
    return NextResponse.json(
      { success: false, data: null, error: "INTERNAL_ERROR", message: "Erro ao registrar manutenção." },
      { status: 500 },
    );
  }
}
